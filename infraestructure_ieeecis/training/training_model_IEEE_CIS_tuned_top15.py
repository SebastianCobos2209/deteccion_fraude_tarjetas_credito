"""
================================================================================
SCRIPT: training_model_IEEE_CIS_tuned_top15.py
PROPOSITO: Entrenamiento del Isolation Forest acotado a las Top 15 Variables.
TIPO DE ARCHIVO: Entrenamiento / Modelado (Experimental)
================================================================================

DESCRIPCION:
Este script entrena un modelo Isolation Forest utilizando exclusivamente las
15 variables predictivas mas relevantes seleccionadas para evaluar el trade-off
de latencia y precision. Sirvio para demostrar experimentalmente que reducir
a solo 15 variables baja la latencia a ~0.10ms pero reduce significativamente
el F1-Score (a ~0.10), sentando las bases para expandirlo a 25 variables.

ORDEN DE EJECUCION (FASE DE EXPERIMENTACIÓN Y ANÁLISIS):
...
4. EJECUTAR ESTE SCRIPT (`training_model_IEEE_CIS_tuned_top15.py`) de forma experimental para comparar métricas.
...
# Definimos las variables pactadas para el simulador
TOP_15_FEATURES = [
...
# Limpieza y Filtrado de TOP 15
def clean_data(df: pd.DataFrame):
    print("[2/6] Filtrando únicamente las Top 15 variables predictivas...")
 
    target = df["isFraud"]
    
    # Filtrar solo las 15 columnas seleccionadas
...
def train_model(X_train, y_train):
    print("[4/6] Entrenando Isolation Forest con Top 15 variables...")
...
def save_artifacts(model, feature_names: list):
    ...
    joblib.dump(model, MODEL_FILE.replace(".pkl", "_top15.pkl"))
    print(f"  Modelo guardado como: {MODEL_FILE.replace('.pkl', '_top15.pkl')}")
 
    # Guardar las variables que el simulador NECESITA saber que existen
    features_path = FEATURES_FILE.replace(".json", "_top15.json")
...
def main():
    print("=" * 60)
    print(" Entrenamiento IEEE-CIS Fraud Detection - MODO TOP 15 VARIABLES")
    print("=" * 60)
    ...
    print("\nEntrenamiento Top 15 completado. Entregable listo para debatir.")
